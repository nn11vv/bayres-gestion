...
function PresupuestosTab() {
  ...
  const load = useCallback(async () => {
    try { setLoading(true); setData(await dbGet("presupuestos")); } catch(e: any) { setErr(e.message); } finally { setLoading(false); }
  }, []);
  useEffect(() => { load(); }, [load]);

  const filtered  = filter === "all" ? data : data.filter((p: any) => p.estado === filter);
  const totalAcep = data.filter((p: any) => p.estado==="aceptado").reduce((a: number, p: any) => a+Number(p.importe||0), 0);
  const nPend     = data.filter((p: any) => p.estado==="pendiente").length;

  const openNew  = () => { setEditing(null); setForm(blank); setShowForm(true); };
  const openEdit = (p: any) => { setEditing(p); setForm({...p}); setShowForm(true); setDetail(null); };
  const submit   = async () => {
    if (!form.cliente.trim()) return;
    setSaving(true);
    try {
      if (editing) { const u = await dbUpdate("presupuestos", editing.id, form); setData((d: any[]) => d.map(p => p.id===editing.id ? u : p)); }
      else         { const u = await dbInsert("presupuestos", form);              setData((d: any[]) => [u, ...d]); }
      setShowForm(false);
    } catch(e: any) { setErr(e.message); } finally { setSaving(false); }
  };
  const del = async (id: string) => {
    try { await dbDelete("presupuestos", id); setData((d: any[]) => d.filter(p => p.id!==id)); setDetail(null); } catch(e: any) { setErr(e.message); }
  };
  const changeEstado = async (id: string, estado: string) => {
    try { await dbUpdate("presupuestos", id, { estado }); setData((d: any[]) => d.map(p => p.id===id ? {...p,estado} : p)); setDetail((d: any) => d ? {...d,estado} : null); } catch(e: any) { setErr(e.message); }
  };
    ...
    <div>
      ...
      {loading ? <Spinner /> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {filtered.map((p: any) => (
            <div key={p.id} onClick={() => setDetail(p)} style={{ background: "#0A1F4E", border: "1px solid #1A3A7A", borderRadius: 14, padding: 14, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ fontWeight: 700, fontSize: 15, color: "#EEF2FF" }}>{p.cliente}</span>
                <span style={{ fontWeight: 800, fontSize: 15, color: ACCENT }}>{p.importe ? `${p.importe}€` : "—"}</span>
              </div>
              <div style={{ fontSize: 13, color: "#7AA0D4", marginBottom: 8 }}>{p.servicio} · {p.zona}</div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <Badge estado={p.estado} map={ESTADOS_P} />
                <span style={{ fontSize: 12, color: "#3A5A9A" }}>{fmt(p.fecha)}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {detail && (
        <Modal title="Presupuesto" onClose={() => setDetail(null)}>
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: "#EEF2FF", marginBottom: 4 }}>{detail.cliente}</div>
            <div style={{ fontSize: 14, color: "#7AA0D4" }}>{detail.servicio} · {detail.zona}</div>
          </div>
          <MapsLink direccion={detail.direccion} />
          <div style={{ background: "#0D2259", borderRadius: 10, padding: 14, marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#7AA0D4", fontSize: 13 }}>Fecha</span><span style={{ color: "#EEF2FF", fontSize: 13 }}>{fmt(detail.fecha)}</span></div>
          </div>
          {detail.nota && <Note text={detail.nota} />}
          <Field label="Cambiar estado">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
              {Object.entries(ESTADOS_P).map(([k,v]) => (
                <button key={k} onClick={() => changeEstado(detail.id, k)} style={{ border: `1px solid ${v.color}55`, borderRadius: 8, padding: "8px 6px", background: detail.estado===k ? v.color+"22" : "transparent", color: v.color, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{v.label}</button>
              ))}
            </div>
          </Field>
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            <button onClick={() => openEdit(detail)} style={{ ...S.btnGhost, flex: 1 }}>✏️ Editar</button>
            <button onClick={() => del(detail.id)}   style={{ ...S.btnGhost, flex: 1, color: "#DC2626", borderColor: "#DC262640" }}>🗑️ Borrar</button>
          </div>
        </Modal>
      )}
      {showForm && (
        <Modal title={editing ? "Editar presupuesto" : "Nuevo presupuesto"} onClose={() => setShowForm(false)}>
          <Field label="Cliente"><input style={S.input} value={form.cliente} onChange={e=>setForm({...form,cliente:e.target.value})} placeholder="Nombre del cliente" /></Field>
          <Field label="Zona"><select style={S.select} value={form.zona} onChange={e=>setForm({...form,zona:e.target.value})}>{ZONAS.map(z=><option key={z}>{z}</option>)}</select></Field>
          <DireccionField value={form.direccion||""} onChange={v=>setForm({...form,direccion:v})} />
          <Field label="Servicio"><select style={S.select} value={form.servicio} onChange={e=>setForm({...form,servicio:e.target.value})}>{SERVICIOS.map(s=><option key={s}>{s}</option>)}</select></Field>
          <Field label="Importe (€)"><input style={S.input} type="number" value={form.importe} onChange={e=>setForm({...form,importe:e.target.value})} placeholder="0" /></Field>
          <Field label="Estado"><select style={S.select} value={form.estado} onChange={e=>setForm({...form,estado:e.target.value})}>{Object.entries(ESTADOS_P).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</select></Field>
          <Field label="Fecha"><input style={S.input} type="date" value={form.fecha} onChange={e=>setForm({...form,fecha:e.target.value})} /></Field>
          <Field label="Nota"><textarea style={{...S.input,minHeight:70,resize:"vertical"}} value={form.nota} onChange={e=>setForm({...form,nota:e.target.value})} placeholder="Recordatorio, observaciones..." /></Field>
...
function MaterialesTab() {
  ...
  const load = useCallback(async () => {
    try { setLoading(true); setData(await dbGet("materiales")); } catch(e: any) { setErr(e.message); } finally { setLoading(false); }
  }, []);
  useEffect(() => { load(); }, [load]);

  const pendientes = data.filter((m: any) => !m.comprado);
  const comprados  = data.filter((m: any) =>  m.comprado);
  const urgentes   = pendientes.filter((m: any) => m.urgente).length;
  ...
  const toggle = async (id: string, field: string) => {
    const prev = data.find((m: any) => m.id===id);
    const val  = !prev[field];
    try {
      await dbUpdate("materiales", id, { [field]: val });
      setData((d: any[]) => d.map(m => m.id===id ? {...m,[field]:val} : m));
      if (field==="urgente" && val) { if (notifOk) fireNotif("⚡ Material urgente — Bayres", `${prev.item} marcado como urgente`); showBanner(prev.item); }
    } catch(e: any) { setErr(e.message); }
  };
  const del = async (id: string) => {
    try { await dbDelete("materiales", id); setData((d: any[]) => d.filter(m => m.id!==id)); } catch(e: any) { setErr(e.message); }
  };
  const submit = async () => {
    ...
    <div>
      ...
      {banner && (
        <div style={{ background: "#7F1D1D", border: "1px solid #DC2626", borderRadius: 12, padding: "12px 16px", marginBottom: 12, display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 18 }}>⚡</span>
          <div><div style={{ fontWeight: 700, fontSize: 13, color: "#FCA5A5" }}>Material urgente</div><div style={{ fontSize: 12, color: "#F87171" }}>{banner} — aviso enviado</div></div>
        </div>
      )}
      {!notifOk && (
        <button onClick={async () => setNotifOk(await requestPush())} style={{ width: "100%", background: "#0D2259", border: "1px dashed #1E7FD8", borderRadius: 12, padding: "10px 14px", marginBottom: 14, color: "#7AA0D4", fontSize: 13, cursor: "pointer", textAlign: "left" }}>
      ...
      {loading ? <Spinner /> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {shown.map((m: any) => (
            <div key={m.id} style={{ background: "#0A1F4E", border: `1px solid ${m.urgente&&!m.comprado?"#DC262640":"#1A3A7A"}`, borderRadius: 14, padding: 14, display: "flex", alignItems: "center", gap: 12 }}>
              <button onClick={() => toggle(m.id,"comprado")} style={{ width:28,height:28,borderRadius:"50%",border:`2px solid ${m.comprado?"#059669":"#1A3A7A"}`,background:m.comprado?"#059669":"transparent",cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:"#fff" }}>
                {m.comprado?"✓":""}
              </button>
              <div style={{ flex:1,minWidth:0 }}>
                {m.urgente&&!m.comprado&&<div style={{ fontSize:10,color:"#DC2626",fontWeight:700,letterSpacing:0.5,marginBottom:2 }}>⚡ URGENTE</div>}
                <div style={{ fontWeight:700,fontSize:15,color:m.comprado?"#3A5A9A":"#EEF2FF",textDecoration:m.comprado?"line-through":"none" }}>{m.item}</div>
                <div style={{ fontSize:13,color:"#4A6A9A" }}>{m.cantidad}{m.nota?` · ${m.nota}`:""}</div>
              </div>
              <div style={{ display:"flex",gap:6 }}>
                {!m.comprado&&<button onClick={()=>toggle(m.id,"urgente")} style={{ background:m.urgente?"#DC262620":"#0D2259",border:`1px solid ${m.urgente?"#DC262660":"#1A3A7A"}`,borderRadius:8,padding:"6px 8px",cursor:"pointer",fontSize:14 }}>⚡</button>}
                <button onClick={()=>del(m.id)} style={{ background:"#0D2259",border:"1px solid #1A3A7A",borderRadius:8,padding:"6px 8px",cursor:"pointer",fontSize:14 }}>🗑️</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {showForm && (
        <Modal title="Añadir material" onClose={() => setShowForm(false)}>
          ...
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:18,background:"#0D2259",borderRadius:10,padding:"12px 14px",border:"1px solid #1A3A7A" }}>
            <input type="checkbox" id="urgente" checked={form.urgente} onChange={e=>setForm({...form,urgente:e.target.checked})} style={{ width:18,height:18,cursor:"pointer",accentColor:"#DC2626" }} />
...
function TrabajosTab() {
  ...
  const load = useCallback(async () => {
    try { setLoading(true); setData(await dbGet("trabajos")); } catch(e: any) { setErr(e.message); } finally { setLoading(false); }
  }, []);
  useEffect(() => { load(); }, [load]);

  const filtered = filter==="all" ? data : data.filter((t: any) => t.estado===filter);
  const enCurso  = data.filter((t: any) => t.estado==="en_curso").length;
  const hoy      = data.filter((t: any) => t.fecha===new Date().toISOString().slice(0,10)).length;

  const openNew  = () => { setEditing(null); setForm(blank); setShowForm(true); };
  const openEdit = (t: any) => { setEditing(t); setForm({...t}); setShowForm(true); setDetail(null); };
  const submit   = async () => {
    if (!form.cliente.trim()) return;
    setSaving(true);
    try {
      if (editing) { const u = await dbUpdate("trabajos", editing.id, form); setData((d: any[]) => d.map(t => t.id===editing.id?u:t)); }
      else         { const u = await dbInsert("trabajos", form);              setData((d: any[]) => [u,...d]); }
      setShowForm(false);
    } catch(e: any) { setErr(e.message); } finally { setSaving(false); }
  };
  const del = async (id: string) => {
    try { await dbDelete("trabajos",id); setData((d: any[]) => d.filter(t=>t.id!==id)); setDetail(null); } catch(e: any) { setErr(e.message); }
  };
  const changeEstado = async (id: string, estado: string) => {
    try { await dbUpdate("trabajos",id,{estado}); setData((d: any[]) => d.map(t=>t.id===id?{...t,estado}:t)); setDetail((d: any)=>d?{...d,estado}:null); } catch(e: any) { setErr(e.message); }
  };
    ...
    <div>
      ...
      {loading ? <Spinner /> : (
        <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
          {filtered.length===0&&<Empty />}
          {filtered.map((t: any) => (
            <div key={t.id} onClick={()=>setDetail(t)} style={{ background:"#0A1F4E",border:"1px solid #1A3A7A",borderRadius:14,padding:14,cursor:"pointer" }}>
              <div style={{ display:"flex",justifyContent:"space-between",marginBottom:6 }}>
                <span style={{ fontWeight:700,fontSize:15,color:"#EEF2FF" }}>{t.cliente}</span>
                <span style={{ fontSize:12,color:"#3A5A9A" }}>{fmt(t.fecha)}</span>
              </div>
              <div style={{ fontSize:13,color:"#7AA0D4",marginBottom:8 }}>{t.servicio} · {t.zona}</div>
              <Badge estado={t.estado} map={ESTADOS_T} />
            </div>
          ))}
        </div>
      )}
      ...
      {detail && (
        <Modal title="Trabajo" onClose={()=>setDetail(null)}>
          <div style={{ marginBottom:16 }}>
            <div style={{ fontSize:20,fontWeight:800,color:"#EEF2FF",marginBottom:4 }}>{detail.cliente}</div>
            <div style={{ fontSize:14,color:"#7AA0D4" }}>{detail.servicio} · {detail.zona} · {fmt(detail.fecha)}</div>
          </div>
          <MapsLink direccion={detail.direccion} />
          {detail.nota&&<Note text={detail.nota} />}
          <Field label="Cambiar estado">
      ...
      {showForm && (
        <Modal title={editing?"Editar trabajo":"Nuevo trabajo"} onClose={()=>setShowForm(false)}>
          ...
          <DireccionField value={form.direccion||""} onChange={v=>setForm({...form,direccion:v})} />
          <Field label="Servicio"><select style={S.select} value={form.servicio} onChange={e=>setForm({...form,servicio:e.target.value})}>{SERVICIOS.map(s=><option key={s}>{s}</option>)}</select></Field>
          <Field label="Estado"><select style={S.select} value={form.estado} onChange={e=>setForm({...form,estado:e.target.value})}>{Object.entries(ESTADOS_T).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</select></Field>
          <Field label="Fecha"><input style={S.input} type="date" value={form.fecha} onChange={e=>setForm({...form,fecha:e.target.value})} /></Field>
          <Field label="Nota"><textarea style={{...S.input,minHeight:70,resize:"vertical"}} value={form.nota} onChange={e=>setForm({...form,nota:e.target.value})} placeholder="Observaciones, acceso, materiales..." /></Field>
