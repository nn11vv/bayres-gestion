...
function notificarUrgenteTodos() {
  fetch("/api/notificar", { method: "POST" }).catch(console.error);
}
async function getNumeroDoc(tipo: "PR" | "FV"): Promise<string> {
  ...
  return d.numero || `${tipo}-${new Date().getFullYear()}-0001`;
}
...
async function exportarTodo() {
  ...
  const content = `PRESUPUESTOS\n${toCSV(presupuestos)}\n\nMATERIALES\n${toCSV(materiales)}\n\nTRABAJOS\n${toCSV(trabajos)}`;
  const blob = new Blob([content], { type: "text/csv;charset=utf-8;" });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement("a"); a.href = url; a.download = `bayres-backup-${new Date().toISOString().slice(0,10)}.csv`; a.click();
...
const calcTotal = (importe: unknown, tieneIva: unknown) => { const b = Number(importe||0); return tieneIva ? b*(1+IVA) : b; };
...
function IvaDesglose({ importe, tieneIva }: { importe: unknown; tieneIva: unknown }) {
  ...
  const base = Number(importe), iva = base*IVA, total = base+iva;
  return (
    <div style={{ background:"#0D2259", borderRadius:10, padding:"10px 14px", marginBottom:14, border:"1px solid #1A3A7A" }}>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}><span style={{ color:"#7AA0D4", fontSize:13 }}>Base</span><span style={{ color:"#EEF2FF", fontSize:13 }}>{base.toFixed(2)}€</span></div>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}><span style={{ color:"#7AA0D4", fontSize:13 }}>IVA 21%</span><span style={{ color:"#EEF2FF", fontSize:13 }}>{iva.toFixed(2)}€</span></div>
      <div style={{ display:"flex", justifyContent:"space-between", borderTop:"1px solid #1A3A7A", paddingTop:6 }}><span style={{ color:"#EEF2FF", fontSize:14, fontWeight:700 }}>Total</span><span style={{ color:ACCENT, fontSize:16, fontWeight:800 }}>{total.toFixed(2)}€</span></div>
    </div>
  );
}
  ...
  return (
    <Field label="Dirección">
      <div style={{ display:"flex", gap:8 }}>
        ...
        {value.trim() && <a href={mapsUrl(value)} target="_blank" rel="noreferrer" style={{ background:ACCENT, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", width:42, flexShrink:0, textDecoration:"none", fontSize:18 }}>📍</a>}
      </div>
    </Field>
  );
}
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:300, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ background:"#0A1F4E", borderRadius:16, padding:24, maxWidth:360, width:"100%", border:"1px solid #1A3A7A" }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={onConfirm} style={{ ...S.btnPrim, flex:1, background:"#DC2626" }}>Confirmar</button>
        </div>
      </div>
    </div>
  );
...
function FilterPills({ options, active, onChange }: { options: [string,string][]; active: string; onChange: (k:string)=>void }) {
  return (
    <div style={{ display:"flex", gap:6, overflowX:"auto", marginBottom:14, paddingBottom:4 }}>
      {options.map(([k,label]) => <button key={k} onClick={()=>onChange(k)} style={{ border:"none", borderRadius:20, padding:"5px 12px", fontSize:12, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap", background:active===k?ACCENT:"#0D2259", color:active===k?"#fff":"#7AA0D4" }}>{label}</button>)}
    </div>
  );
}
...
function FacturaModal({ registro, tipo, onClose }: { registro: Record<string,unknown>; tipo: "PR"|"FV"; onClose: ()=>void }) {
  ...
  useEffect(() => {
    if (registro.numero_doc) { setNumeroDoc(registro.numero_doc as string); return; }
    getNumeroDoc(tipo).then(n => {
      setNumeroDoc(n);
      dbUpdate(tipo === "PR" ? "presupuestos" : "trabajos", registro.id as string, { numero_doc: n }).catch(console.error);
    });
  }, [registro, tipo]);

  const factura: Record<string,unknown> = {
    numeroDoc, tipo, fecha: registro.fecha, cliente: registro.cliente,
    direccionCliente, nifCliente, servicio: registro.servicio,
    ...
    <Modal title="Generar factura" onClose={onClose} zIndex={200}>
      ...
      <div style={{ background:"#0D2259", borderRadius:10, padding:"10px 14px", marginBottom:16, border:"1px solid #1A3A7A" }}>
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
          <span style={{ color:"#7AA0D4", fontSize:13 }}>Número</span>
          <span style={{ color:"#EEF2FF", fontSize:13, fontWeight:700 }}>{numeroDoc || "Generando..."}</span>
        </div>
        <div style={{ display:"flex", justifyContent:"space-between" }}>
          <span style={{ color:"#7AA0D4", fontSize:13 }}>Total</span>
          <span style={{ color:ACCENT, fontSize:15, fontWeight:800 }}>{calcTotal(registro.importe, registro.tiene_iva).toFixed(2)}€</span>
        </div>
      </div>

      <Field label="Dirección del cliente (opcional)">
        <input style={S.input} value={direccionCliente} onChange={e=>setDireccionCliente(e.target.value)} placeholder="Calle, número, ciudad" />
      </Field>
      <Field label="NIF del cliente (opcional)">
        <input style={S.input} value={nifCliente} onChange={e=>setNifCliente(e.target.value)} placeholder="Si está vacío aparece 'Contado'" />
      </Field>
      <Field label="Email del cliente (para envío)">
...
function PresupuestosTab({ onCrearTrabajo }: { onCrearTrabajo: (p: Record<string,unknown>) => void }) {
  ...
  const blank = { cliente:"", zona:ZONAS[0], direccion:"", servicio:SERVICIOS[0], importe:"", estado:"pendiente", fecha:new Date().toISOString().slice(0,10), nota:"", tiene_iva:false, email_cliente:"", direccion_cliente:"", nif_cliente:"" };
  ...
  const del = async (id: string) => {
    try { await dbDelete("presupuestos",id); setData(d=>d.filter(p=>p.id!==id)); setDetail(null); setConfirmDel(null); } catch(e) { setErr((e as Error).message); }
  };
    ...
    <div>
      ...
      {confirmTrabajo && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ background:"#0A1F4E", borderRadius:16, padding:24, maxWidth:360, width:"100%", border:"1px solid #1A3A7A" }}>
            <div style={{ fontSize:15, color:"#EEF2FF", marginBottom:8, fontWeight:700 }}>✅ Presupuesto aceptado</div>
            <div style={{ fontSize:14, color:"#7AA0D4", marginBottom:20 }}>¿Crear un trabajo a partir de este presupuesto?</div>
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={()=>setConfirmTrabajo(null)} style={{ ...S.btnGhost, flex:1 }}>No por ahora</button>
              <button onClick={()=>{ onCrearTrabajo(confirmTrabajo!); setConfirmTrabajo(null); setDetail(null); }} style={{ ...S.btnPrim, flex:1 }}>Crear trabajo</button>
            </div>
          </div>
        </div>
      )}
      {showFactura && <FacturaModal registro={showFactura} tipo="PR" onClose={()=>setShowFactura(null)} />}
      ...
      {detail && (
        <Modal title="Presupuesto" onClose={()=>setDetail(null)}>
          ...
          <div style={{ background:"#0D2259", borderRadius:10, padding:14, marginBottom:14 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}><span style={{ color:"#7AA0D4", fontSize:13 }}>Base (sin IVA)</span><span style={{ color:"#EEF2FF", fontSize:13 }}>{detail.importe?`${Number(detail.importe).toFixed(2)}€`:"—"}</span></div>
            {detail.tiene_iva && detail.importe && <>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}><span style={{ color:"#7AA0D4", fontSize:13 }}>IVA 21%</span><span style={{ color:"#EEF2FF", fontSize:13 }}>{(Number(detail.importe)*IVA).toFixed(2)}€</span></div>
              <div style={{ display:"flex", justifyContent:"space-between", borderTop:"1px solid #1A3A7A", paddingTop:6, marginTop:4 }}><span style={{ color:"#EEF2FF", fontSize:14, fontWeight:700 }}>Total con IVA</span><span style={{ color:ACCENT, fontSize:16, fontWeight:800 }}>{(Number(detail.importe)*(1+IVA)).toFixed(2)}€</span></div>
            </>}
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:detail.tiene_iva?8:0 }}><span style={{ color:"#7AA0D4", fontSize:13 }}>Fecha</span><span style={{ color:"#EEF2FF", fontSize:13 }}>{fmt(detail.fecha as string)}</span></div>
          </div>
          ...
          <div style={{ display:"flex", gap:8, marginTop:8 }}>
            ...
            <button onClick={()=>setShowFactura(detail)} style={{ ...S.btnGhost, flex:1 }}>🧾 Factura</button>
      ...
      {showForm && (
        <Modal title={editing?"Editar presupuesto":"Nuevo presupuesto"} onClose={()=>setShowForm(false)}>
          ...
          <Field label="Importe base (sin IVA)"><input style={S.input} type="number" value={form.importe as string} onChange={e=>setForm({...form,importe:e.target.value})} placeholder="0" /></Field>
...
function MaterialesTab() {
  ...
  const toggle = async (id: string, field: string) => {
    const prev = data.find(m=>m.id===id); if (!prev) return;
    const val  = !prev[field];
    try {
      await dbUpdate("materiales",id,{[field]:val});
      setData(d=>d.map(m=>m.id===id?{...m,[field]:val}:m));
      if (field==="urgente" && val) { if (notifOk) fireNotif("⚡ Material urgente — Bayres",`${prev.item as string} marcado como urgente`); showBanner(prev.item as string); notificarUrgenteTodos(); }
    } catch(e) { setErr((e as Error).message); }
  };
  ...
  const submit = async () => {
    if (!form.item.trim()) return; setSaving(true);
    try {
      const u = await dbInsert("materiales",form); setData(d=>[u,...d]);
      if (form.urgente) { if (notifOk) fireNotif("⚡ Material urgente — Bayres",`${form.item} añadido como urgente`); showBanner(form.item); notificarUrgenteTodos(); }
      setShowForm(false); setForm({ item:"", cantidad:"", urgente:false, comprado:false, nota:"" });
    } catch(e) { setErr((e as Error).message); } finally { setSaving(false); }
  };
...
function TrabajosTab({ precargar }: { precargar: Record<string,unknown>|null }) {
  ...
  const [confirmDel, setConfirmDel] = useState<string|null>(null);
  const [showFactura, setShowFactura] = useState<Record<string,unknown>|null>(null);
  const blank = { cliente:"", zona:ZONAS[0], direccion:"", servicio:SERVICIOS[0], estado:"pendiente", fecha:new Date().toISOString().slice(0,10), nota:"", hora_inicio:"", email_cliente:"", direccion_cliente:"", nif_cliente:"" };
  const [form, setForm] = useState<Record<string,unknown>>(blank);
            ...
            <div key={t.id as string} onClick={()=>setDetail(t)} style={{ background:"#0A1F4E",border:"1px solid #1A3A7A",borderRadius:14,padding:14,cursor:"pointer" }}>
              <div style={{ display:"flex",justifyContent:"space-between",marginBottom:6 }}>
                ...
                <div style={{ textAlign:"right" }}>
                  ...
                  {t.hora_inicio && <span style={{ display:"block",fontSize:11,color:"#7AA0D4" }}>🕐 {t.hora_inicio as string}</span>}
                </div>
              </div>
  ...
  return (
    <div style={{ background:NAVY2, minHeight:"100vh", fontFamily:"system-ui,sans-serif", color:"#EEF2FF", maxWidth:480, margin:"0 auto", paddingBottom:80 }}>
      ...
      <div style={{ padding:"16px 16px 0" }}>
        {tab==="presupuestos" && <PresupuestosTab onCrearTrabajo={handleCrearTrabajo} />}
